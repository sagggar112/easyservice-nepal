...
const getProviderServices = async (providerId) => {
  const result = await pool.query(
    `SELECT ps.id, ps.provider_id, ps.service_id, ps.price, ps.is_active,
            s.service_name, s.description, s.base_price, s.duration_minutes,
            c.category_name
...
const updateProviderService = async ({ providerId, serviceId, price, durationMinutes }) => {
  const values = [providerId, serviceId];
  const fields = [];
  ...
  if (!fields.length) throw new Error("Provide a price or duration to update.");
  const result = await pool.query(`UPDATE provider_services ps SET ${fields.join(", ")}, updated_at=CURRENT_TIMESTAMP FROM services s WHERE ps.provider_id=$1 AND ps.service_id=$2 AND s.id=ps.service_id RETURNING ps.*, s.service_name, s.duration_minutes`, values);
  return result.rows[0];
};

const deactivateProviderService = async ({ providerId, serviceId }) => {
  const result = await pool.query(
    `UPDATE provider_services SET is_active = FALSE, updated_at = CURRENT_TIMESTAMP WHERE provider_id = $1 AND service_id = $2 RETURNING *`,
...
const setAvailability = async ({ providerId, dayOfWeek, startTime, endTime }) => {
  const result = await pool.query(
    `INSERT INTO provider_availability (provider_id, day_of_week, start_time, end_time, is_available)
     VALUES ($1, $2, $3, $4, TRUE)
     ON CONFLICT (provider_id, day_of_week, start_time, end_time) DO UPDATE SET is_available = TRUE
     RETURNING *`,
...
const getAvailability = async (providerId) => {
  const result = await pool.query(
    `SELECT id, provider_id, day_of_week, start_time, end_time, is_available FROM provider_availability WHERE provider_id = $1 AND is_available = TRUE ORDER BY day_of_week, start_time`,
    [providerId]
  );
  return result.rows;
};
