...
const createBooking = async ({ customer_id, provider_id, service_id, booking_date, booking_time, address, notes, total_amount }) => {
  ...
  try {
    ...
    const service = await client.query("SELECT id, duration_minutes FROM services WHERE id=$1", [service_id]);
    ...
    // Reject overlapping active bookings using the actual service duration.
    const conflict = await client.query(
      `SELECT b.id FROM bookings b
       ...
       WHERE b.provider_id=$1 AND b.booking_date=$2
         ...
         AND b.booking_time < ($3::time + ($4 || ' minutes')::interval)
...
const getBookingById = async (id, user) => {
  ...
  const result = await pool.query(`SELECT b.* FROM bookings b WHERE b.id=$1 ${access}`, values); return result.rows[0];
};
...
const updateBookingStatus = async (id, status, user) => {
  ...
  if (event && updated.customer_id) await createNotification({ userId: updated.customer_id, bookingId: updated.id, type: event, title: { Accepted: "Booking accepted", "In Progress": "Service in progress", Completed: "Service completed", Cancelled: "Booking cancelled" }[status], message: `Your booking #${updated.id} is now ${status.toLowerCase()}.` });
  return updated;
};
